...
class BeforeandAfter(AbstractAnim):
    ...
    def plant(self):
        ...
        # Create soil patches
        soil_patches = VGroup(*[Ellipse(width=0.6, height=0.2, color=DARK_BROWN, fill_opacity=1) for _ in range(3)])
        soil_patches.arrange(RIGHT, buff=1.5)
        soil_patches.next_to(plants, DOWN, buff=0.05)
        ...
        question_text = Text("As per the above picture, which are more and which are less - flower\npots or plants? Put for your answer.", 
                             font_size=28, color=BLUE).next_to(plants, DOWN *4, buff=0.1)

        # Create answer boxes
        answer_boxes = VGroup(*[Square(side_length=0.4, color=PURPLE) for _ in range(4)])
        answer_boxes[0:2].arrange(RIGHT, buff=2)
        answer_boxes[2:4].arrange(RIGHT, buff=3)
        answer_boxes.arrange(DOWN*1, buff=0.5)
        answer_boxes.next_to(question_text, DOWN, buff=1)
        ...
        more_less_labels[2].next_to(answer_boxes[2], UP, buff=0.2)
        more_less_labels[3].next_to(answer_boxes[3], UP, buff=0.2)

        # Create images
        pot_image = pots[0].copy().scale(2).next_to(answer_boxes[0], LEFT, buff=1)
        plant_image = create_plant().scale(1.5).next_to(answer_boxes[2], LEFT, buff=1.5)
    ...
    def cranes(self):
        # Create cranes
        def create_crane():
            body = Line(start=ORIGIN, end=UP*0.5, color=WHITE)
            neck = Arc(radius=0.3, angle=PI/2, color=WHITE).next_to(body, UP, buff=0)
            beak = Triangle(color=WHITE).scale(0.1).next_to(neck, UP+RIGHT, buff=0)
            leg = Line(start=body.get_bottom(), end=body.get_bottom()+DOWN*0.3, color=WHITE)
            return VGroup(body, neck, beak, leg)
        ...
        # Create fish
        def create_fish():
            body = Arc(radius=0.3, angle=PI, color=ORANGE)
            tail = Triangle(color=ORANGE).scale(0.2).next_to(body, LEFT, buff=0)
            eye = Dot(color=WHITE).scale(0.5).move_to(body.point_from_proportion(0.8) + UP*0.1)
        ...
        question = Text("How many cranes are there and how many fishes are there? Are they\nin equal number?", 
                        font_size=33, color=BLUE).to_edge(UP)
 ...
 # Add numbers
        ...
        fish_numbers = Text("Fishes", font_size=36,color=ORANGE).next_to(boxes[1], DOWN*-0.9+RIGHT*6.3)
    ...
    def Less(self):
        ...
        # Create the "equal" text
        equal_text = Text("Less", color=WHITE).scale(0.8).next_to(arrows, DOWN)
    ...
    def More(self):
        ...
        # Create the left set
        left_set = Circle(radius=1.8, color=WHITE)
        left_dots = VGroup(*[Sphere(color=ORANGE) for _ in range(4)]).scale(0.42)
        ...
        # Create the "equal" text
        equal_text = Text("More", color=WHITE).scale(0.8).next_to(arrows, DOWN)
...
###
    def G(self):
        ...
        title = Text("There are 3 numbers in each row, Draw round the least number.", font_size=36)
        title.to_edge(UP)
        self.play(Write(title))
    ...
    def Next(self):
        ...
        # Create title
        title = Text("Write the next number in the circle", color=PINK).to_edge(UP)
        self.play(Write(title))
    ...
    def middle(self):
        ...
        # Create title
        title = Text("Write the middle number in the circle", color=BLUE).to_edge(UP)
        self.play(Write(title))
    ...
    def previous(self):
        ...
        # Create title
        title = Text("Write the previous number in the circle", color=ORANGE).to_edge(UP)
        self.play(Write(title))
        ...
        # Animate answers
        for i, row in enumerate(rows):
            circle = Circle(radius=0.3, color=PINK).move_to(row[0].get_center())
    ...
    def Bigger(self):
        ...
        title = Text("Tick the bigger of the two numbers given below.", font_size=36)
        title.to_edge(UP)
        self.play(Write(title))
    ...
    def Biggeramoung3(self):
        ...
        title = Text("Tick the biggest of the 3 numbers given below.", font_size=36,color=YELLOW)
        title.to_edge(UP)
        self.play(Write(title))
    ...
    def counting(self):
        ...
        car_row = create_row(Rectangle, 4, Rectangle, 3, BLUE, "(d)")
        
        # Arrange rows
        exercise = VGroup(biscuit_row, cherry_row, mango_row, car_row).arrange(DOWN, buff=0.7).scale(0.7).shift(DOWN*1)
