...
class AbstractAnim(Scene):
    ...
    def construct1(self,cvo,cvoParent):
        ...
        if not cvo.isEllipse:
            ...
            if (cvo.IsMathText):
                cname = MathTex(cvo.cname,color=self.colorChoice[colorChoiceIndex]).move_to(c1nameposition).shift(UP * 0.25).tex_template=TexFontTemplates.epigrafica
            else:
                cname = Tex(cvo.cname,color=self.colorChoice[colorChoiceIndex]).move_to(c1nameposition).shift(UP * 0.25).tex_template=TexFontTemplates.epigrafica
            ...
            if (cvo.IsMathText):
                oname = MathTex(cvo.oname,color=self.colorChoice[colorChoiceIndex]).move_to(o1nameposition).shift(UP * 0.15).tex_template=TexFontTemplates.epigrafica
            else:
                oname = Tex(cvo.oname,color=self.colorChoice[colorChoiceIndex]).move_to(o1nameposition).shift(UP * 0.15).tex_template=TexFontTemplates.epigrafica
