...
class Grade7Ch13AreaAndPerimeter(AbstractAnim):
    ...
    def rhombus(self):  
        ...
        self.colorChoice=[RED_A,BLUE_A,BLUE_B,ORANGE,WHITE]
        self.angleChoice = [TAU/4,TAU/4,TAU/4,-TAU/4]
        p2=cvo.CVO().CreateCVO("Area Formula", "(d1*d2)/2").setPosition([1.5,2,0])
        ...
        p6=cvo.CVO().CreateCVO("Area of Rhombus", "(7.5*5.6)/2 = 21").setPosition([-1,-0.5,0])
        p2.cvolist.append(p3)
        p2.cvolist.append(p4)
        p2.cvolist.append(p5)
        p5.cvolist.append(p6)
    ...
    def rectanglurpath(self):
        ...
        t1 = Text("Rectangular Paths",font_size=25,color=PINK)
        t1.move_to([-5.5,3.5, 0])
        self.play(Write(t1))
        
        t2 = Text("Example:  A  plot  is  60m  long  and 40m  wide. A  path is 3m  wide is to  be  constructed around the plot.",font_size=22)
        t2.move_to([0,3, 0])
        self.play(Write(t2))
        t2 = Text("Find  Area  of  path.",font_size=22)
        t2.move_to([-5.5,2.5, 0])
        self.play(Write(t2))
        self.wait(1.5)
        ...
        self.wait(1)
        t3 = MathTex(r"\text{Solution : Let ABCD be the given plot. A 3m wide path is running all around it . }",font_size=25)
        t3.move_to([2.4,2, 0])
        self.play(Write(t3))
        self.wait(1.5)
        t3 = MathTex(r"\text{To  find area of path we have to subtract the area of the smaller}",font_size=25)
        t3.move_to([2.7,1.5, 0])
        self.play(Write(t3))
        self.wait(1.5)
        t3 = MathTex(r"\text{rectangle ABCD from the area of the bigger rectangle EFGH.}",font_size=25)
        t3.move_to([2.7,1, 0])
        self.play(Write(t3))
        self.wait(1.5)
        t3 = MathTex(r"->\text{inner  rectangle ABCD : }length=60m, breadth=40m",font_size=25)
        t3.move_to([2.3,0.5, 0])
        self.play(Write(t3))
        self.wait(1.5)
        t4 = MathTex(r"->\text{area= }60 \times 40 = 2400 m^2",font_size=25)
        t4.move_to([0.5,0, 0])
        self.wait(1.5)
        self.play(Write(t4))
        t5 = MathTex(r"->\text{path width=}3m",font_size=25)
        t5.move_to([0,-0.5, 0])
        self.play(Write(t5))
        self.wait(1.5)
        t6 = MathTex(r"->\text{outer  rectangle EFGH:} length=60m+(3+3)m= 66m",font_size=25)
        t6.move_to([2.2,-1, 0])
        self.play(Write(t6))
        self.wait(1.5)
        t6 = MathTex(r"\text{breadth}=40m(3+3)m= 46m",font_size=25)
        t6.move_to([3.8,-1.5, 0])
        self.play(Write(t6))
        self.wait(1.5)
        t7 = MathTex(r"->\text{area}=66 \times 46=3036 m^2",font_size=25)
        t7.move_to([0.5,-2, 0])
        self.play(Write(t7))
        self.wait(1.5)
        t8 = MathTex(r"->\text{area of path}= (3036-2400)m^2= 636m^2",font_size=25)
        t8.move_to([1.5,-2.5, 0])
        self.play(Write(t8))
        self.wait(2)
