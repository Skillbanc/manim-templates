...
class Grade6Chapter5MeasuresOfLinesAndAngles(AbstractAnim):
    ...
    def Lines(self):
        ...
        # self.isRandom = False
        p10=cvo.CVO().CreateCVO("Lines","").setPosition([0,2,0])
        p11=cvo.CVO().CreateCVO("Parallel lines","do not intersect at any point").setPosition([3,-2,0]).setangle(-TAU/3)
        p12=cvo.CVO().CreateCVO("Intersecting lines","lines meet at a point").setPosition([-3,-2,0]).setangle(TAU/3)
    ...
    def display_ray(self):
        ...
        # Create the start point and define the end position
        start_point = Dot(ORIGIN)
        end_position = RIGHT * 3
        
        # Create the ray from the start point to the end position
        ray = Arrow(start_point.get_center(), end_position, color=GREEN, buff=0)
        
        # Create labels
        ray_label = Text("Ray").next_to(ray, UP)
        start_label = MathTex("A").next_to(start_point, LEFT)
        end_label = MathTex("B").next_to(end_position, RIGHT)
        info = Text("AB is a Ray", font_size=24).next_to(ray, DOWN)
        
        # Animate the ray and its labels
        self.play(Create(start_point))
        self.play(Create(ray), Write(ray_label))
        self.play(Write(start_label), Write(end_label))
        self.play(Write(info))
        self.wait(1)
        self.play(FadeOut(start_point), FadeOut(ray), FadeOut(ray_label),
                  FadeOut(start_label), FadeOut(end_label), FadeOut(info), FadeOut(title))
    def display_line(self):
        ...
        # Create the line segment as a DoubleArrow
        line = DoubleArrow(start_position, end_position, color=RED)
        
        # Create labels
        line_label = Text("Line").next_to(line, UP)
        start_label = MathTex("A").next_to(start_position, LEFT)
        end_label = MathTex("B").next_to(end_position, RIGHT)
        info = Text("AB is a Line", font_size=24).next_to(line, DOWN)
        
        # Animate the line and its labels
        self.play(Create(line), Write(line_label))
        self.play(Write(start_label), Write(end_label))
        self.play(Write(info))
        self.wait(1)
        self.play(FadeOut(line), FadeOut(line_label), FadeOut(start_label),
                  FadeOut(end_label), FadeOut(info), FadeOut(title))
    ...
    def Complementary(self):
        # self.positionChoice=[[0,2,0],[2,0,0],[-2,0,0]]
        # self.isRandom = False
        p10=cvo.CVO().CreateCVO("Complementary Angle","").setPosition([0,2,0])
        p11=cvo.CVO().CreateCVO("Property",r"$Adjacent \quad angles \quad sum \quad is \quad 90^\circ$").setPosition([3,-2,0]).setangle(-TAU/3)
        p12=cvo.CVO().CreateCVO("Example",r"$60^\circ,30^\circ$").setPosition([-3,-2,0]).setangle(TAU/3)
        p10.setcircleradius(1.5)
        p11.setcircleradius(1.75)
        p12.setcircleradius(1.75)
        p10.cvolist.append(p11)
    ...
    def ComplementaryAnim(self):
        ...
        # Position the right angle diagram in the center
        ...
        ).arrange(DOWN, aligned_edge=LEFT).scale(0.8).next_to(right_angle, DOWN, buff=0.5)

        # Display the explanation text below the animation
        self.play(Write(explanation_text))

        # Highlight the complementary relationship
        box_30 = SurroundingRectangle(label_30, color=YELLOW, buff=0.1)
        box_60 = SurroundingRectangle(label_60, color=YELLOW, buff=0.1)
    ...
    def Supplementary(self):
        # self.positionChoice=[[0,2,0],[2,0,0],[-2,0,0]]
        # self.isRandom = False
        p10=cvo.CVO().CreateCVO("Supplementary Angle","").setPosition([0,2,0])
        p11=cvo.CVO().CreateCVO("Property",r"$Adjacent \quad angles \quad sum \quad is \quad 180^\circ$").setPosition([3,-2,0]).setangle(-TAU/3)
        p12=cvo.CVO().CreateCVO("Example",r"$70^\circ,110^\circ$").setPosition([-3,-2,0]).setangle(TAU/3)
        p10.setcircleradius(1.5)
        p11.setcircleradius(1.75)
        p12.setcircleradius(1.75)
        p10.cvolist.append(p11)
...
if __name__ == "__main__":
    scene = Grade6Chapter5MeasuresOfLinesAndAngles()
    scene.render()