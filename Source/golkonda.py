...
class golkonda(AbstractAnim):
    ...
    def introduction(self):
       # Define text for the introduction
        ...
        # Transform the introduction text to the first expectation text
        self.play(Transform(intro_text, text1), run_time=2)
        self.wait(4)
        
        # Transform the first expectation text to the second expectation text
        self.play(Transform(intro_text, text2), run_time=2)
        self.wait(4)
        
        # Transform the second expectation text to the third expectation text
        self.play(Transform(intro_text, text3), run_time=2)
        self.wait(4)
    ...
    # Define heading for the amount to be paid to the bus agency
        heading = Text("Q1:Amount to be paid to the bus agency", font_size=20, color=BLUE)
        heading.to_edge(UP + LEFT)
        self.play(FadeIn(heading))
        p10=cvo.CVO().CreateCVO("distance ","112km").setPosition([-4.5,1,0]).setangle(-TAU/4)
        p11=cvo.CVO().CreateCVO("cost per km","24 INR").setPosition([-2.2,1,0]).setangle(-TAU/4)
        p12=cvo.CVO().CreateCVO("Total bus cost","112km x 24INR/km\n= 2688INR").setPosition([-4.5,-2,0]).setangle(-TAU/4)
        ...
        # Define heading for the amount to be paid to the bus agency
        heading1 = Text("Q2:Tickets at Golkonda Fort", font_size=20, color=BLUE)  # Simplified text
        heading1.to_edge(UP + RIGHT)
        self.play(FadeIn(heading1))
        p10=cvo.CVO().CreateCVO("total people ","64").setPosition([1.5,1,0])
        p11=cvo.CVO().CreateCVO("ticket cost per person","40 INR").setPosition([4.6,1,0])
        p12=cvo.CVO().CreateCVO("Total ticket cost","64 people x 40 INR/person\n= 2560 INR").setPosition([1.5,-2,0]).setangle(-TAU/4)
        p10.cvolist.append(p11)
        p11.cvolist.append(p12)
    ...
    def intro2(self):    
        ...
        heading.to_edge(UP + LEFT)
        self.play(FadeIn(heading))
        p10=cvo.CVO().CreateCVO("travel cost per person","2688 INR / 64 people\n=42 INR ").setPosition([0,2,0])
        p11=cvo.CVO().CreateCVO("(1)travel cost","42 INR").setPosition([-3,0,0])
        p12=cvo.CVO().CreateCVO("(2) ticket cost","40 INR").setPosition([0,0,0]).setangle(-TAU/4)
        p13=cvo.CVO().CreateCVO("(3) additional cost","110 INR").setPosition([3,0,0]).setangle(-TAU/4)
        p14=cvo.CVO().CreateCVO("Total cost per person","40 + 42= 102 INR ").setPosition([-1.5,-2.5,0]).setangle(-TAU/4)
        p10.cvolist.append(p11)
    ...
    def daytrip(self):
        ...
        # Fade in box, question, and answer
        self.play(Create(qa_box_c), Write(question_c), Write(answer_c))
        self.wait(1)
        ...
        heading = Text("Years", font_size=24)
        heading.next_to(timeline, UP+LEFT)  # Position the heading above the timeline
    ...
    def intro3(self):
        ...
        p15 = cvo.CVO().CreateCVO("Nagina Bagh", "").setPosition([-3, -3, 0])
    ...
    def intro4(self):
        # Define heading for the amount to be paid to the bus agency
        heading = Text("Q3:If a square shaped patch has a 2m side then,\nwhat is its perimeter?", font_size=20, color=BLUE)
        heading.to_edge(UP + LEFT)
        self.play(FadeIn(heading))
        p10=cvo.CVO().CreateCVO("formula ","4 x s").setPosition([-4.5,1,0]).setangle(-TAU/4)
        p11=cvo.CVO().CreateCVO("side (s)","2m").setPosition([-2.2,1,0]).setangle(-TAU/4)
        p12=cvo.CVO().CreateCVO("perimeter","4 x 2m = 8m").setPosition([-3.2,-2,0]).setangle(-TAU/4)
        p10.cvolist.append(p11)
        p11.cvolist.append(p12)
        ...
        # Define heading for the amount to be paid to the bus agency
        heading1 = Text("Q4:If this patch is lined with bricks of length 25 cm each,\nthen how many such bricks are needed to line the patch?", font_size=20, color=BLUE)
        heading1.to_edge(UP + RIGHT)
        self.play(FadeIn(heading1))
        p10=cvo.CVO().CreateCVO("length of patch perimeter ","4 x 2m = 8m").setPosition([1.5,1,0]).setangle(-TAU/4)
        p11=cvo.CVO().CreateCVO("no.of bricks","8m/0.25m=32").setPosition([4.6,1,0]).setangle(-TAU/4)
        p10.cvolist.append(p11)
    ...
    def intro5(self):
         # Define heading for the amount to be paid to the bus agency
        heading = Text("Q5:If there are 13 such patches\nthen how many bricks are needed?", font_size=20, color=BLUE)
        heading.to_edge(UP + LEFT)
        self.play(FadeIn(heading))
        p10=cvo.CVO().CreateCVO("no.of bricks ","32").setPosition([-4.5,1,0]).setangle(-TAU/4)
        p11=cvo.CVO().CreateCVO("total no.of bricks","13 x 32 = 416").setPosition([-2.2,1,0]).setangle(-TAU/4)
        p10.cvolist.append(p11)
        
        self.construct1(p10,p10)
        
        # Define heading for the amount to be paid to the bus agency
        heading1 = Text("Q6:If the rectangular shaped patch is 3 m long\nand 1m 50 cm wide then,\nwhat is its perimeter?", font_size=20, color=BLUE)
        heading1.to_edge(UP + RIGHT)
        self.play(FadeIn(heading1))
        p10=cvo.CVO().CreateCVO("formula","2(l+b)").setPosition([1.5,1,0]).setangle(-TAU/4)
        p11=cvo.CVO().CreateCVO("l,b","3,1.5").setPosition([4.6,1,0]).setangle(-TAU/4)
        p12=cvo.CVO().CreateCVO("perimeter","2(3+1.5)=9m").setPosition([1.5,-2,0]).setangle(-TAU/4)
        p10.cvolist.append(p11)
        p11.cvolist.append(p12)
    ...
    def intro6(self):
          # Define heading for the amount to be paid to the bus agency
        heading = Text("Q7:If this patch is lined with bricks of length 25 cm,\nthen how many such bricks are\nneeded to line the patch?", font_size=20, color=BLUE)
        heading.to_edge(UP + LEFT)
        self.play(FadeIn(heading))
        p12=cvo.CVO().CreateCVO("perimeter","9m").setPosition([-4.5,-2,0]).setangle(-TAU/4)
        p13=cvo.CVO().CreateCVO("no.of bricks","9m/0.25m=36").setPosition([-2.2,-2,0]).setangle(-TAU/4)
        p12.cvolist.append(p13)
        
        self.construct1(p12,p12)
        
        # Define heading for the amount to be paid to the bus agency
        heading1 = Text("Q8:If there are 19 such patches\nthen how many bricks are needed?", font_size=20, color=BLUE)
        heading1.to_edge(UP + RIGHT)
        self.play(FadeIn(heading1))
        p10=cvo.CVO().CreateCVO("no.of bricks","36").setPosition([1.5,1,0]).setangle(-TAU/4)
        p11=cvo.CVO().CreateCVO("total number of bricks","19 x 36= 684").setPosition([4,1,0]).setangle(-TAU/4)
        p10.cvolist.append(p11)
    ...
    def intro7(self):
         # Define heading for the amount to be paid to the bus agency
        heading = Text("Q9:Estimate the perimeter of your school boundary.\nAbout how many times more is the perimeter of the fort?", font_size=20, color=BLUE)
        heading.to_edge(UP)
        self.play(FadeIn(heading))
        p10=cvo.CVO().CreateCVO("school boundary","l=1km,b=1km").setPosition([-4.7,1,0]).setangle(-TAU/4)
        p11=cvo.CVO().CreateCVO("perimeter","2(1+1)=4km").setPosition([-2,1,0]).setangle(-TAU/4)
        ...
        p12=cvo.CVO().CreateCVO("fort boundary ","l=30km,b=20km").setPosition([1.2,1,0]).setangle(-TAU/4)
        p13=cvo.CVO().CreateCVO("perimeter","2(30+20)=100km").setPosition([4.6,1,0]).setangle(-TAU/4)
        p12.setcircleradius(1.2)
        p13.setcircleradius(1.5)
        p12.cvolist.append(p13)
        
        self.construct1(p12,p12)      
        
        p14=cvo.CVO().CreateCVO("square,fort comparission ","fort perimeter/school perimeter=25 times").setPosition([0,-2.5,0])
